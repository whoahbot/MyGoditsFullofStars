(function() {
  document.write("<canvas id='stars' width='600px' height='150px'></canvas>")
  var canvas = document.getElementById('stars');
  var ctx = canvas.getContext('2d');


  var background = ctx.createLinearGradient(0,0,0,600);
  background.addColorStop(0, 'black');
  background.addColorStop(0.4, '#113');

  ctx.fillStyle = background;
  ctx.fillRect(10, 10, 600, 150);

  var starGradient = ctx.createRadialGradient(5,5,1,10,10,4);
  ...
  for(var i = 0; i <= (Math.floor(Math.random() * 10000) + 100); i++) {
    ...
    ctx.strokeStyle = 'rgba(' + color + ',' + color + ',' + color +',0.4)';
    ctx.stroke();
  }
